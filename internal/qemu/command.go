...
// a [Error] with the guest flag unset is returned. If the guest
// returned an error or failed a [Error] with guest flag set is
// returned.
func (c *Command) Run(
	ctx context.Context,
	stdin io.Reader,
	...
	consolePipes, writers, err := c.consolePipes()
	if err != nil {
		return err
	}

	c.stdoutParser.Dst = stdout
	...
	stdoutPipe := pipe.New("stdout", &c.stdoutParser, stdoutR, stdoutW)
	consolePipes.Run(stdoutPipe)
	...
	pipesErr := consolePipes.Wait(ctx)
...
func (c *Command) consolePipes() (*pipe.Pipes, []*os.File, error) {
	writers := []*os.File{}
	guestPipes := &pipe.Pipes{}

	for idx, path := range c.additionalConsoles {
		...
		writers = append(writers, fifoStop)

		decoder := pipe.Decoder(outputFile)
		hostPipe := pipe.New(pipe.Path(idx), decoder, fifo, fifoStop)
		guestPipes.Run(hostPipe)
	}
