	...
	tests := []struct {
		...
		},
		{
			...
			expect: []Argument{
				...
				RepeatableArg("chardev", "stdio,id=con0"),
				RepeatableArg("device", "virtconsole,chardev=con0"),
				RepeatableArg("chardev", "file,id=con1,path=/dev/fd/3"),
				RepeatableArg("device", "virtconsole,chardev=con1"),
				RepeatableArg("chardev", "file,id=con2,path=/dev/fd/4"),
		...
		},
		{
			...
			expectedCmd: &Command{
				cmd: exec.CommandContext(
					...
					"-serial", "chardev:con0",
					"-chardev", "file,id=con1,path=/dev/fd/3",
					"-serial", "chardev:con1",
					"-display", "none",
					"-monitor", "none",
					"-no-reboot",
