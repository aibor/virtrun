...
func serialConsolesConnected() ([]console, error) {
	serials, err := os.Open(serialConsoleInfoFile)
	if err != nil {
		return nil, fmt.Errorf("read info: %w", err)
	}
	defer serials.Close()

	return serialConsolesConnectedFromReader(serials)
}

// serialConsolesConnectedFromReader returns a slice of serial consoles
...
func serialConsolesConnectedFromReader(r io.Reader) ([]console, error) {
	consoles := []console{}
	...
	for scanner.Scan() {
		...
		// 	...
		_, err := fmt.Sscanf(scanner.Text(), "%d: uart:%s port:", &port, &uart)
		if err != nil || uart == "unknown" {
			continue
		}
	...
	return consoles, scanner.Err() //nolint:wrapcheck
}

func consolePath(typ string, id int) string {
	return fmt.Sprintf("/dev/%s%d", typ, id)
}
