...
// Close is a no-op to implement the [pipe.Consumer] interface.
func (*stdoutParser) Close() error {
	return nil
}

// ReadFrom reads from the given reader which is expected to be output from a
// guest's standard output.
//
// Once done [stdoutParser.Result] can be called to retrieve the result.
func (p *stdoutParser) ReadFrom(src io.Reader) (int64, error) {
	var read int64
...
//
// It is only valid to call after [stdoutParser.ReadFrom] ran and terminated
// successfully.
