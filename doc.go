// Package virtrun provides a simple way for running go tests in an isolated
...
// can only be used if the target architecture matches the virtrun binary
...
//
//	$ go test -exec 'go run github.com/aibor/virtrun/cmd/virtrun' .
//
...
// [virtrun.Tests]. You may keep this in a separate test file and use
...
// additional ones. See [virtrun.Tests] for the steps it does.
//
...
// Or build and run on the fly with "go run":
//
//	$ go test -tags virtrun -exec 'go run github.com/aibor/virtrun/cmd/virtrun -standalone' .
//
// There is also support for coverage profiles. Just specify it as usual:
//
...
// architecture.
//
//	$ GOARCH=arm64 go test -exec "virtrun -kernel $(realpath kernel/vmlinuz.arm64) -standalone" .
package virtrun
